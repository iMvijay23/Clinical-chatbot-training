    ...
    def on_step_end(
        ...
        **kwargs,
        ...
        if (state.global_step + 1) % self.save_steps == 0:
            ...
            # Create a unique saving directory based on the global step
            save_directory = os.path.join(args.output_dir, f"checkpoint-{state.global_step}")

            if self.trainer.accelerator.is_main_process:
                unwrapped_model.save_pretrained(save_directory, state_dict=state_dict)
            self.trainer.accelerator.wait_for_everyone()
        return control
...
def create_and_prepare_model(args):
    ...
    peft_config = None
    #print(set(model.hf_device_map.values())) # iadded this to see what the device map is, causes error with deepspeed  
